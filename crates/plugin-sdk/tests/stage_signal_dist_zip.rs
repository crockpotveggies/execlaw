...
fn dist_signal_zip_stages_cleanly() {
    ...
    let workspace_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .parent()
        .unwrap();
    let zip_path = workspace_root.join("dist").join("signal-0.3.5.zip");
    if !zip_path.exists() {
        ...
        eprintln!(
            "dist/signal-0.3.5.zip not present at {}; skipping",
            zip_path.display()
        );
        return;
    }
    ...
    assert_eq!(staged.manifest.plugin.version, "0.3.5");
