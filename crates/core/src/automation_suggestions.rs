//! Automation suggestions Ã¢â‚¬â€ the discovery surface for the
...
//!   * Threshold: Ã¢â€°Â¥ `MIN_EVENT_COUNT` distinct events.
...
//! `(kind, source, status)` keeps the sweep idempotent Ã¢â‚¬â€ re-running
...
/// Default sweep cadence Ã¢â‚¬â€ once per day. Suggestions are a discovery
...
/// many events in the window to be worth suggesting. Conservative Ã¢â‚¬â€
...
pub enum SuggestionStatus {
    /// Active suggestion Ã¢â‚¬â€ surfaces on the landing page.
    Pending,
...
impl<'a> SuggestionStore<'a> {
    ...
    /// upserted (created OR refreshed). Pure-ish Ã¢â‚¬â€ `now_unix` is
    ...
    pub fn sweep(&self, now_unix: i64) -> Result<usize, SuggestionError> {
        ...
        for cand in candidates {
            ...
            // match is the right signal Ã¢â‚¬â€ once you have a Ring
    ...
    pub fn collect_candidates(
        ...
        // Stable order Ã¢â‚¬â€ biggest first so the editor lands the most
    ...
    /// suggestion. Callers are the deferred agent-drafting path Ã¢â‚¬â€
...
fn derive_suggested_name(kind: BusEventKind, source: &str) -> String {
    // Strip the namespace prefix from sources like `webhook:ring` Ã¢â€ â€™
...
mod tests {
    ...
    fn sweep_surfaces_high_volume_pattern_with_no_matching_automation() {
        ...
        // Seed 15 events from `ring` at "now" Ã¢â‚¬â€ passes threshold.
        let now = 1_000_000;
    ...
    fn sweep_skips_low_volume_pattern() {
        ...
        let store = SuggestionStore::new(&db);
        let now = 1_000_000;
        // 9 events Ã¢â‚¬â€ under the threshold of 10.
    ...
    fn sweep_is_idempotent_and_refreshes_count() {
        ...
        let pending = store.list_pending().unwrap();
        // Still one suggestion Ã¢â‚¬â€ refreshed in place.
        assert_eq!(pending.len(), 1);
        assert!(
    ...
    fn dismiss_mutes_pattern_and_blocks_future_sweep() {
        ...
        // Re-sweep Ã¢â‚¬â€ must not resurface the dismissed pattern.
        let bus = BusEventStore::new(&db);
        for i in 100..110 {
            bus.publish(
                &BusEvent {
