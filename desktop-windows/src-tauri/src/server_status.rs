...
//!
//! Any non-200 / no-connect maps to `Stopped` (the SCM service
//! probably hasn't come up yet) or `Error` (server is up but
//! malfunctioning).
//!
//! This file is a verbatim port of
//! `desktop-macos/src-tauri/src/server_status.rs` — keep them in
//! lockstep, the semantics MUST stay identical so a Windows operator
//! and a macOS operator see the same status strings.

use std::time::Duration;

/// User-facing state for the tray status row. The variants serialise
/// to short strings that fit in a tray tooltip / menu label.
...
/// a sluggish first boot when the service is launching any sidecar
/// containers, short enough that the tray feels responsive when the
/// server is just down.
