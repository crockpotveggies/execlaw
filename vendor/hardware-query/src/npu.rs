...
// execlaw vendoring fix: upstream gates this import to `target_os =
// "linux"` only, but `Command::new("sysctl")` is also called inside
// the `target_os = "macos"` block in `detect_apple_neural_engine`
// (line ~420). Widen the cfg to any unix so macOS builds compile.
// Upstream issue tracked at https://github.com/ciresnave/hardware-query
#[cfg(unix)]
