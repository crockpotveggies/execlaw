...
**Naming history:** originally drafted as the "Bridge supervisor" because every supervised companion container in tree today bridges a transport (Signal-cli, WhatsApp, Matrix). Renamed 2026-05-04 — "sidecar" is the established container-pattern term and accommodates non-transport companions (OCR workers, ffmpeg pools) that are likely to follow. The supervisor still happens to manage only transport-bridging sidecars today; the manifest's `[services.sidecar]` `channel` field exists because of that, and will become optional + paired with a `kind` discriminator when the first non-transport sidecar lands.
...
- `sidecar_supervisor` *(this)* — owns plugin-managed sidecar containers that today mediate inbound/outbound traffic with a third-party messaging system (Signal first; WhatsApp, Matrix, Telegram natural follow-ons). Future non-transport sidecars land here too.
...
2. **Inbound message ingestion** — long-poll or websocket subscription against the sidecar's local RPC, decode each event, route to the right `principal_group_id` (or trigger first-contact intake). *(Phase 3 work.)*
3. **Outbound dispatch RPC** — exposed as a `TransportApi` capability that plugin tools (`signal.send_message`, `signal.reply`) consume via `ToolCtx`. *(Phase 3 work.)*
4. **Identity + foreign-id storage** — `state_transport_bindings` keyed by `(channel, foreign_id)`. Reused on every outbound dispatch and on inbound routing.
5. **Healthcheck → alert fan-out** — when a sidecar goes down, fingerprinted alert (`sidecar.signal.unreachable`) using the existing alert subsystem. *(Phase 3 work.)*
...
- `transport_id` arg renamed to `channel` everywhere (matches the storage column + `state_principal_groups.channel`).
...
- All methods return `Result<_, ApiError>`; transport-specific failure modes collapse to existing variants (`NotFound`, `Storage`, `NotAuthorized`).
- Capability is a single flat `Capability::Transport`; the per-call `channel` arg dispatches inside the impl. Earlier sketch said `transport:signal` per-channel — flattening is cleaner because channel is data, not capability.

## Storage

New table — see `crates/core/migrations/0032_transport_bindings.sql` for the live schema. Notes vs the original sketch:

- The column is `channel`, not `transport_id` — matches the existing `state_principal_groups.channel` column so joins read naturally.
- PK is `(channel, foreign_id)` — inbound routing is the dominant hot path. The reverse `principal_group_id → bindings` direction is covered by `idx_transport_bindings_pg`.
- A separate `idx_transport_bindings_channel_last_seen` keeps the retention sweep off a full scan once the table grows.
- No FK to `state_principal_groups` — see migration comment + the `insert_succeeds_for_nonexistent_principal_group` test for why (first-contact flow needs binding-first/group-second ordering).
...
- `insert_binding` errors-as-`Ok(false)` on PK conflict (does NOT silently steal). `repoint_binding` is the explicit rebind path and returns the displaced `principal_group_id` for audit logging. Earlier draft had a single `upsert_binding` that conflated the two — first-insert and "steal an existing binding" were syntactically identical, exactly the kind of footgun a buggy sidecar would trip on.

## Sidecar RPC

Each sidecar container exposes a localhost-only HTTP+WS endpoint. Schema we settle on:
...
The sidecar meta lives on the `[[services]]` entry rather than `[transport]` — see `SidecarMeta` in `crates/plugin-sdk/src/manifest.rs`. The path is `[services.sidecar]` with fields `channel`, `rpc_port`, `rpc_health_path` (defaults to `/healthz`). One sidecar per channel across all installed plugins; the manifest validator + the hook registry both enforce this.
...
- container name format: `execlaw-sidecar-<plugin_id>-<channel>` — mirrors `execlaw-backend-<purpose>`
...
- **Status-change event spam.** First draft published `SidecarStatusChanged` on every reconcile pass even when status was unchanged. Centralised via `transition_status(channel, slot, new_status)` which dedups + publishes only on transitions.
...
| 2b | Sidecar supervisor skeleton (container lifecycle reusing `ServiceController`, healthcheck/restart loop, no RPC yet) | **shipped** |
...
1. Does the sidecar supervisor share `ServiceController` with `backend_supervisor`, or get its own? (Lean toward sharing — same image-pull + lifecycle plumbing.)
...
3. Multi-account: one sidecar container per Signal number, or one container handling many? selfhosted-claw was single-account — we should plan for multi but not implement it in Phase 3.
