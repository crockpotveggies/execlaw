...
Last update: 2026-05-01 (Phase 9 plugin-tier pivot — first real plugin shipped). The binary-plugin approach landed first (google-contacts as a 2.88 MB Rust binary on top of the new OAuth machinery) but proved too heavy for HTTP-API-wrapper plugins — multi-minute cold compiles per plugin, ~50 MB of duplicated TLS/HTTP across the inventory. Reverted that and built an embedded Rhai script tier instead: plugins are now `.rhai` source files (~180 lines for the real google-contacts), zero compile, ~5 KB ZIP on disk, hot-reloadable. Manifest gained `runtime.tier = "script" | "subprocess"`; new `execlaw-script` crate provides the sandboxed engine (1M ops/call, depth 64, sized strings/arrays/maps, **SSRF guard on HTTP primitives** matching the native WebFetchTool) and the host-injected primitives (HTTP via ureq, JSONPath via `serde_json_path`, string/hash/time helpers, logging into tracing). Plugin-host dispatch is tier-aware: `resolve_identity` and `call_tool` try the subprocess map first, fall through to the script map, with the same OAuth `_oauth` injection working unchanged across both tiers. The reusable OAuth machinery (migration 0028, vault stores, provider trait, admin endpoints, refresh sweeper) carried over from the binary attempt. Drive-by: fixed a real `SubprocessPlugin::shutdown` deadlock (parked RPCs now drain on stdout EOF) that affected every existing subprocess plugin.

**First real script-tier plugin shipped: `plugin-google-contacts`.** ~180 lines of Rhai (vs ~400 lines of Rust + a 2.88 MB binary in the binary attempt). Same surface (Google People API → identity_provider for email + phone, contacts.list tool, OAuth via the controller account). 13 plugin-side tests (load real main.rhai, mock People API, exercise every match path) + 1 server-side end-to-end test (real ZIP install → host dispatch → resolve_identity → contacts.list). Operator-facing Settings → Google Contacts page restored alongside, unchanged from the binary version (the OAuth UX didn't move). Re-bucketed plugin-inventory.md: the actual queue shrank from ~13 plugins to **4 Rhai + 4 subprocess** after marking what's already-native (url-fetch, search-*, research-*, core-tools) and what's defer-to-MCP (GitHub, Slack, Notion, etc.).
...
- `cargo test --workspace --lib` — **1117 passing, 0 failing**
- `cargo test --workspace --tests` — **1179 passing, 0 failing** across lib + integration + plugin bin tests (script_plugin_install + google_contacts_e2e end-to-end install + dispatch coverage)
- `cargo bench --workspace --no-run` — **clean** (≈75 benches; new `script_hot_paths` pins per-plugin engine-build at ~99 µs, real-plugin parse at ~261 µs, dispatch overhead at ~12 µs per call)
- `cd web && npm test` — **323 passing** (includes 5 GoogleContactsPage tests against the restored SPA settings page)
