...
- `cargo test --workspace --no-fail-fast` — **516 passing, 0 failing** (+7: 1 UserStore set_password_hash + 5 password-rotation route tests, plus net adjustments from Phase 8.6 Login consolidation)
- `cargo bench --workspace --no-run` — **clean** (60 benches across 9 crates)
- `cd web && npm test` — **130 passing** (-5 obsolete users-page/hardware tests removed; +5 LoginPage; +1 backends hardware section)
- `cd web && npm run build` — **clean** (~330 KB JS / 311 KB CSS, both well under budget)
