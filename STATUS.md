...
- `cargo test --workspace --tests` — **1162 passing, 0 failing** across lib + integration + plugin bin tests (now includes `script_plugin_install` end-to-end test that ZIPs a .rhai plugin in memory + drives identity.resolve through the host)
- `cargo bench --workspace --no-run` — **clean** (≈72 benches)
- `cd web && npm test` — **318 passing**
- `cd web && npm run build` — **clean** (≈682 KB JS / 335 KB CSS)
- **First real script-tier plugin:** TBD — the infrastructure is ready; google-contacts.rhai is the natural first port (~50 lines)
