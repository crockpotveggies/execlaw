...
Last update: 2026-04-30 (Phase 9 ignition), after the first plugin to ride the new generic OAuth machinery — `plugin-google-contacts` — landed end-to-end: migration 0028 (`state_oauth_clients` / `state_oauth_tokens` / `state_oauth_pending`), reusable `core::oauth` stores, `oauth_provider::GoogleOauthProvider` (authorize URL / code exchange / refresh / userinfo), Controller-only admin endpoints + unauthenticated `GET /api/oauth/{provider}/callback`, 60s refresh sweeper, `PluginHost.oauth_tokens_for(plugin_id)` injection of access_tokens into outgoing JSON-RPC params under a reserved `_oauth` key, the plugin itself (Google People API; identity provider for email + phone, `contacts.list` tool), and a Settings → Google Contacts SPA page. Hot-path perf: HookRegistry now caches `[[oauth_accounts]]` at enable-time so dispatch skips the manifest TOML re-parse — single-account lookup is ~98 ns (down from multi-µs), miss is ~11 ns.
...
- `cargo test --workspace --lib` — **1090 passing, 0 failing**
- `cargo test --workspace --tests` — **1141 passing, 0 failing** across lib + integration + plugin bin tests (approval_flow, plugin_lifecycle, runner_round_trip, voice_ws_round_trip; runner_e2e_docker is `#[ignore]` by design)
- `cargo bench --workspace --no-run` — **clean** (≈72 benches; new oauth_accounts_for bench pins single-account lookup at ~98 ns and miss at ~11 ns, validating the HookRegistry cache supersedes the per-call manifest re-parse)
- `cd web && npm test` — **323 passing** (+5 GoogleContactsPage tests)
